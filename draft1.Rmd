...
This paper provides an introduction to the \pkg{regsem} package, outlining the mathematical details of the regularized SEM method and the usage of the package itself. 
...
With highly constrained structural equation models, achieving convergence can be particularly problematic in using \pkg{regsem}. For instance, with the latent change score model (cite jack), Bayesian regularization methods have less difficulty in reaching convergence across chains (cite chapter). With the advent of additional sparsity inducing priors, along with new forms of software such as STAN (cite stan), for some models it may be more appropriate to use these Bayesian regularization methods over their frequentist counterparts. 
